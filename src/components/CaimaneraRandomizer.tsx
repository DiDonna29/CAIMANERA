...
export default function CaimaneraRandomizer() {
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.href = dataUrl;
      link.click();
      toast({ title: "Póster descargado", description: "Imagen guardada." });
    } catch (err) {
          ...
          <div className="space-y-6 animate-in slide-in-from-right-10 fade-in duration-500">
            <div className="text-center space-y-2 pt-8">
              <h1 className="text-4xl md:text-6xl font-black italic tracking-tighter uppercase leading-none">
                ¿QUÉ SE JUEGA<br/><span className="text-primary">HOY?</span>
              </h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {FORMATS.map((f) => (
                <button
                  ...
                  className={cn(
                    "flex flex-col items-center justify-center p-6 rounded-3xl border-2 transition-all duration-300 active:scale-95",
                    playersPerTeam === f.id 
                      ? "bg-primary/20 border-primary shadow-[0_0_20px_rgba(59,130,246,0.2)]" 
                      : "bg-card border-border hover:border-primary/50"
                  )}
                >
                  <div className={cn("mb-3 p-3 rounded-2xl", playersPerTeam === f.id ? "bg-primary text-white" : "bg-muted text-muted-foreground")}>
                    {f.icon}
                  </div>
        ...
        {currentStep === 'players' && (
          <div className="space-y-6 animate-in slide-in-from-right-10 fade-in duration-500">
            <div className="flex items-center justify-between pt-8">
              <Button variant="ghost" onClick={() => setCurrentStep('format')} className="gap-2 font-bold uppercase text-xs italic">
            ...
            <div className="text-center space-y-2">
              <h2 className="text-4xl font-black italic tracking-tighter uppercase leading-none">
                LISTA DE <span className="text-accent">CRACKS</span>
              </h2>
            ...
            <Card className="glass-card border-primary/20 overflow-hidden">
              <CardContent className="p-0">
                <Textarea 
                  ...
                  className="min-h-[300px] border-0 bg-transparent text-xl font-bold italic placeholder:opacity-30 p-6 focus-visible:ring-0 resize-none"
                  value={rosterText}
            ...
            <Button 
              ...
              className="w-full h-16 bg-primary hover:bg-primary/90 text-white text-xl font-black italic tracking-tighter rounded-3xl shadow-xl shadow-primary/20 animate-pulse"
            >
            ...
            {isGenerating ? (
              <div className="h-[60vh] flex flex-col items-center justify-center space-y-8">
                <div className="relative">
                  ...
                  <Trophy className="h-24 w-24 text-primary animate-bounce animate-float" />
                </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between pt-4">
                  <Button variant="outline" onClick={() => setCurrentStep('players')} className="rounded-full h-10 w-10 p-0 border-border bg-card">
                    <ChevronLeft className="h-5 w-5" />
                  </Button>
                  <div className="flex gap-2">
                    <Button onClick={handleDownloadImage} disabled={isDownloading} className="rounded-full bg-primary hover:bg-primary/90 text-white font-black italic text-xs h-10 px-6 gap-2">
                ...
                >
                  <div className="relative bg-[#0a192f] p-6 md:p-10">
                    ...
                    {/* Match Center */}
                    {matchDetails && (
                      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 mb-8 text-center relative overflow-hidden">
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-8 border-t border-white/5 pt-6 text-[10px] font-bold uppercase italic">
                          ...
                          {matchDetails.waitingTeam && (
                            <div className="flex items-center gap-2 justify-center text-primary">
                              <RotateCw className="h-3 w-3 animate-spin-slow" /> ESPERA: {matchDetails.waitingTeam}
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      {generatedTeams.map((team) => (
                        <div key={team.id} className="bg-white/5 rounded-2xl overflow-hidden border border-white/5">
                          <div className="bg-white/5 px-4 py-3 flex justify-between items-center border-b border-white/5">
                            <span className="font-black text-xs text-white italic uppercase">{team.name}</span>
                            <span className="text-[8px] font-black text-white/40">{team.players.length} CRACKS</span>
                          </div>
                          <div className="p-3 space-y-1">
                            {team.players.map((player, pIdx) => (
                              <div key={pIdx} className="flex items-center gap-3 p-2 rounded-lg bg-white/5 border border-white/5 text-[11px] font-black uppercase italic text-white/80">
                ...
                <Button 
                  ...
                  className="w-full text-muted-foreground font-black italic uppercase text-xs hover:text-primary transition-colors"
                >
                  Reiniciar Torneo
                </Button>
              </div>
